...
Notes:
1. If a number does not become Lychrel, all the numbers that it had in
   the intermediate steps should not be Lychrel either. So those numbers
   can be saved and the problem can be shortcutted
"""
...
def is_lychrel(number, step):
    if step > 0 and is_palindrome(number):
        return False
    if step >= 49:
        return True
    return is_lychrel(number + reverse(number), step + 1)
    
total_lychrel_numbers = 0
for i in xrange(1, 10000):
    if (is_lychrel(i, 0)):
        total_lychrel_numbers += 1
print total_lychrel_numbers